...
export default function CatalogPage() {
  ...
  }, [category, color, finish, search, priceRange]);

  const clearFilters = () => { setCategory(''); setColor(''); setFinish(''); setPriceRange([0, 500]); };
  const hasFilters = category || color || finish || priceRange[0] > 0 || priceRange[1] < 500;
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            {products.length} products {search && `matching "${search}"`}
          </p>
        </div>
